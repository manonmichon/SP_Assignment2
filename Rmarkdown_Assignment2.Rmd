...
The chosen physicochemical properties are retrieved from the rcdk github for the selected parsed SMILES. These results are stored in 'molecular_descriptors' as a dataframe, to which the boilingpoints from the wikidata query are added. From this, two subsets are made: a training-set and a test-set.
...
``` {r}
bps_measured = subset_query[,3]; # Renaming column 3 of the subsetted query data to further use and give meaning to this column
smiles = subset_query[,6]; # Renaming column 6 of the subsetted query data to further use and give meaning to this column
...
molecular_descriptors = eval.desc(parsed_smiles, selected_properties); # Request the properties for the selected SMILES
...
The model is constructed using all of the selected molecular descriptors. The amount of components is left empty. The model is validated using a Leave-One-Out validation method.
...
The scatterplot of the model shows the distribution of the training set data along the regression line constructed by the model with 2 components.

``` {r}
PLStest = plsr(bps_measured ~ ., data = desc_train, validation = "LOO"); # Construction of the Partial Least Square model
plot(RMSEP(PLStest), legendpos = "topright", main = "RMSEP plot") # Plotting the error for each amount of latent variables considered
plot(PLStest, ncomp = 2, line = TRUE) # Visualisation of the PLS model in combination with the training data
```

## Testing of the model

Now that the model has been constructed, it can be tested. The predict function from the PLS package will use the previously constructed PLS model and try to predict boiling points based on the molecular descriptors of the data in the test dataset. The number of components used here is 2. This was found to be the optimal amount of components as can be seen in the previous paragraph.
